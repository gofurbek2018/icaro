...
func SendSMSCode(number string, code string, unit models.Unit, auth string, uamip string, uamport string) int {
	...
	if accountSMS.SmsCount <= accountSMS.SmsMaxCount {
		...
		if hotspotCount <= hotspotMaxCountInt || hotspotMaxCountInt == 0 {
			...
			messageType := configuration.Config.Endpoints.Sms.ServiceSid
			urlAPI := "http://195.158.4.114:443/http/send-message"
			query := fmt.Sprintf("username=%s&password=%s&to=%s&message-type=%s&message=%s",
				url.QueryEscape(accountSid),
				url.QueryEscape(authToken),
				url.QueryEscape(number),
				url.QueryEscape(messageType),
				url.QueryEscape(`Login Link: `+GenerateShortURL(configuration.Config.Endpoints.Sms.Link+"?"+auth+"&code="+code+"&num="+url.QueryEscape(number), uamip, uamport, false)+`Codice/Code: `+code+`Logout Link: http://logout`))

			url := fmt.Sprintf("%s?%s", urlAPI, query)
...
func CheckUserIsValid(username string, password string) int {
	// init db instance
	db := database.Instance()
	...
	if userSearch.Id > 0 {
		...
		// search user in history
		var userSearchH models.UserHistory
		db.Where("username = ? and password = ?", username, password).First(&userSearchH)

		// user exists in history
		if userSearchH.Id > 0 {
			return 2 // user is expired
		}
	}
